const contentPrompt = (info, plan) => {
  ...
  return `
    Generate SEO page content using the following inputs:

    Business: ${info.businessBrief}
    Keywords: ${selectedKeywordList}
    Page Type: ${info.pageType}
    Tone: ${info.tone}
    Length: ${info.length} words
    Goal: ${info.goal}
    CTA: ${info.cta}
    Plan: ${plan.name}
    ...
    - 50–100 word intro.
    - Add <img> tags where relevant.
    - Strong CTA at end.
    - Featured snippets if plan ≠ Free.
    - FAQ/HowTo schema if plan = Pro or Agency.
    - Add credibility note.
    - HTML only.
  `
}
const contentSystemPrompt = `You are an expert SEO strategist and professional content writer.

Generate SEO-optimized, crawler-friendly and AI-readable website content that aligns with Google E-E-A-T standards, featured snippet rules and modern search intent patterns.

Follow:
- correct HTML tags (h1, h2, h3, p, ul, table, img)
- natural keyword use
- short paragraphs
- engaging intro
- clean structure for both users and AI agents
- persuasive CTA`

module.exports = { contentPrompt, contentSystemPrompt }